function [Output] = MarmosetGPInfer_CS6_v3(varargin);
...
%indT = find(Output.cleanAnotaton=="EmDisc_CS6" | Output.cleanAnotaton=="EmDisc_CS6_PGC" | Output.cleanAnotaton=="EmDisc_Gast_CS6" | Output.cleanAnotaton=="EmDisc_gast_CS6");
...
%figure(2)
%f2 = patch('Faces',OBJ.objects(4).data.vertices,'Vertices',OBJ.vertices,'FaceVertexCData',m2,'FaceColor','interp','LineStyle','none');

%figure(3)
%f2 = patch('Faces',OBJ.objects(4).data.vertices,'Vertices',OBJ.vertices,'FaceColor',[33, 105, 192]/255,'LineStyle','none','FaceAlpha',.1);
%hold on
%scatter3(Xtrain(indT,1)*Output.scalefactor,Xtrain(indT,2)*Output.scalefactor,Xtrain(indT,3)*Output.scalefactor,45,log(Ytrain(indT)),'o','filled')
%[m2b s2b] = gp(hyp2, 'infExact', @meanConst, @covSEiso, 'likGauss',  Xtrain(indT,1:3), Ytrain(indT)', Xtrain(indT,:));
...
%indT = find(Output.cleanAnotaton=="Stalk_CS6");
hyp2 = HYP2{1,8};
hyp3 = HYP3{1,8};
