function [Output] = MarmosetGP_CS5Cyno(D,Output,gene2test);

%Scale the input data
Xtrain = Output.cynoX / Output.scalefactor;
Ytrain = Output.Ycynotrain;
...
%First is amnion
indT = find(Output.cynoAnotaton=="Am_CS5" | Output.cynoAnotaton=="Am_CS5_PGC");
...
%Second is EmDisk
indT = find(Output.cynoAnotaton=="EmDisc_CS5");
par = {@meanConst,@covSEiso,'likGauss',Xtrain(indT,1:3), Ytrain(indT)'};
hyp2 = feval(@minimize, hyp1, @gp, -1000, im1, par{:});         % optimise
hyp3 = feval(@minimize, hyp1B, @gp, -1000, im2, par{:});         % optimise
HYP2{1,2} = hyp2;
...
%Third is VE
indT = find(Output.cynoAnotaton=="VE_CS5");
par = {@meanConst,@covSEiso,'likGauss',Xtrain(indT,1:3), Ytrain(indT)'};
hyp2 = feval(@minimize, hyp1, @gp, -1000, im1, par{:});         % optimise
hyp3 = feval(@minimize, hyp1B, @gp, -1000, im2, par{:});         % optimise
HYP2{1,4} = hyp2;
...
%Next we have SYS
indT = find(Output.cynoAnotaton=="SYS_CS5");
par = {@meanConst,@covSEiso,'likGauss',Xtrain(indT,1:3), Ytrain(indT)'};
hyp2 = feval(@minimize, hyp1, @gp, -1000, im1, par{:});         % optimise
hyp3 = feval(@minimize, hyp1B, @gp, -1000, im2, par{:});         % optimise
HYP2{1,5} = hyp2;
...
%ExMes
indT = find(Output.cynoAnotaton=="ExMes_CS5");
par = {@meanConst,@covSEiso,'likGauss',Xtrain(indT,1:3), Ytrain(indT)'};
hyp2 = feval(@minimize, hyp1, @gp, -1000, im1, par{:});         % optimise
hyp3 = feval(@minimize, hyp1B, @gp, -1000, im2, par{:});         % optimise
HYP2{1,6} = hyp2;
...
%Tb
indT = find(Output.cynoAnotaton=="Tb_CS5");
par = {@meanConst,@covSEiso,'likGauss',Xtrain(indT,1:3), Ytrain(indT)'};
hyp2 = feval(@minimize, hyp1, @gp, -1000, im1, par{:});         % optimise
hyp3 = feval(@minimize, hyp1B, @gp, -1000, im2, par{:});         % optimise
HYP2{1,7} = hyp2;
...
Output.L1 = [L1_1,L1_2,L1_3,L1_4,L1_5,L1_6,L1_7,L1_8];
Output.L2 = [L2_1,L2_2,L1_3,L2_4,L2_5,L2_6,L2_7,L1_8];
Output.HYP1 = HYP2;
Output.HYP2 = HYP3;
