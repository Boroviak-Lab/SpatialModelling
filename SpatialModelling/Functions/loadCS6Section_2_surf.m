function [Dexp1,Arc,Xtest,Output] = loadCS6Section_2_surf(D3,Dexp)


fid = fopen('CS6/Proj_shots_BLEND_high_update.csv', 'rt'); 
D0_ = textscan(fid,'%f %f %f %s %s','headerLines', 1,'Delimiter',',');
fclose(fid);
D1.data = cell2mat(D0_(:,1:3));
D1.textdata =D0_{:,4};

Arc = importdata('Data/EmDiscCS6_Arc.mat');

OBJ2=importdata('Data/CS6_section_cs2.mat'); %read_wobj('Medium_resolution_CS5_version10-fordylan-310.obj')
...
%Ignore bubble
Xex2 = [OBJ2.vertices(OBJ2.objects(12).data.vertices,:)]; %OBJ2.vertices(OBJ2.objects(32).data.vertices,:)
...
Xtroph2 = [OBJ2.vertices(OBJ2.objects(28).data.vertices,:)];
...
Xsys2 = [OBJ2.vertices(OBJ2.objects(24).data.vertices,:)];
...
%keyboard
DMAP1 = importdata('/Users/christopherpenfold/Desktop/Thorsten/FINAL/CCAFigForESCs/DimRed/PLAXA.csv')
...
h = scatter3(Output.Xtest(Output.ind1,1),Output.Xtest(Output.ind1,2),Output.Xtest(Output.ind1,3),5,'fill', 'MarkerEdgeColor',[7,103,218]/255, 'MarkerFaceColor',[7,103,218]/255)
hold on
...
h = scatter3(Output.Xtest(Output.ind1,1),Output.Xtest(Output.ind1,2),Output.Xtest(Output.ind1,3),5,'fill', 'MarkerEdgeColor',[7,103,218]/255, 'MarkerFaceColor',[7,103,218]/255)
hold on
scatter3(Output.Xtrain(setdiff(find(Output.LabBin==1),[]),1),Output.Xtrain(setdiff(find(Output.LabBin==1),[]),2),Output.Xtrain(setdiff(find(Output.LabBin==1),[]),3),20,'fill','MarkerEdgeColor','k', 'MarkerFaceColor','k')
%scatter3(Output.Xtrain(find(Output.LabBin==1),1),Output.Xtrain(find(Output.LabBin==1),2),Output.Xtrain(find(Output.LabBin==1),3),20,'fill','MarkerEdgeColor','k', 'MarkerFaceColor','k')
%text(Output.Xtrain(find(Output.LabBin==1),1),Output.Xtrain(find(Output.LabBin==1),2),Output.Xtrain(find(Output.LabBin==1),3), CellIDs4(find(Output.LabBin==1)));
%scatter3(Output.Xtest(Output.ind0,1),Output.Xtest(Output.ind0,2),Output.Xtest(Output.ind0,3),5,'fill')
%scatter3(Output.Xtrain(find(Output.LabBin==0),1),Output.Xtrain(find(Output.LabBin==0),2),Output.Xtrain(find(Output.LabBin==0),3),95,'fill')
...
h2 = scatter3(Output.Xtest(Output.ind1,1),Output.Xtest(Output.ind1,2),Output.Xtest(Output.ind1,3),5,'fill', 'MarkerEdgeColor',[7,103,218]/255, 'MarkerFaceColor',[7,103,218]/255)
hold on
