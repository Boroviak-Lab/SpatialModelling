...
saveext = "~/Desktop/Thorsten/FINAL/Mouse3D/"
dir.create(saveext)
...
p <- DimPlot(mouse_data, pt.size = 4, reduction = "umap", label = TRUE, repel = TRUE, do.return = TRUE) 
ggsave(filename=paste(saveext,"/DimRed/UMAP_Type_ClLab","_4.pdf",sep=""),width = 10, height = 8,p)

Idents(mouse_data) <- mouse_data$ID

p <- DimPlot(mouse_data, pt.size = 4, reduction = "umap", label = TRUE, repel = TRUE, do.return = TRUE) +NoLegend()
ggsave(filename=paste(saveext,"/DimRed/UMAP_Type_Labs","_4.pdf",sep=""),width = 45, height = 40,p)
...
FeaturePlot(mouse_data,  reduction = "umap", features = "Sox2", combine=TRUE, cols =  c("lightgrey", "black"), pt.size = 8)
ggsave(filename=paste(saveext,"/DimRed/SOX2","2_4.pdf",sep=""),width = 10, height = 8)

FeaturePlot(mouse_data,  reduction = "umap", features = "Pou5f1", combine=TRUE, cols =  c("lightgrey", "black"), pt.size = 8)
ggsave(filename=paste(saveext,"/DimRed/OCT4","2_4.pdf",sep=""),width = 10, height = 8)

FeaturePlot(mouse_data,  reduction = "umap", features = "Sox17", combine=TRUE, cols =  c("lightgrey", "black"), pt.size = 8)
ggsave(filename=paste(saveext,"/DimRed/SOX17","2_4.pdf",sep=""),width = 10, height = 8)
...
write.csv(as.data.frame(AvExp$RNA), file=paste(saveext,"/AverageExp.csv",sep=""))

Dsubs <- AvExp$RNA[c("Sox2","Pou5f1","Nanog","Prdm14","Fbxo2","Tdgf1","Sox17","Mixl1","Hnf4a","T"),] #GetAssayData(object = AvExp , slot = 'data')[c("Sox2","Pou5f1","Nanog","Prdm14","Fbxo2","Tdgf1","Sox17","Mixl1","Hnf4a","T"),]
